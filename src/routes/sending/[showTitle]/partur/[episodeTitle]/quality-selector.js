...
var HlsQualitySelectorPlugin = (function () {
	...
	HlsQualitySelectorPlugin.prototype.bindPlayerEvents = function bindPlayerEvents() {
		this.player
			...
			.on(
				'addqualitylevel',
				this.onAddQualityLevel.bind(this, this.config.sortAscending, this.config.autoPlacement),
			);
	};
	...
	HlsQualitySelectorPlugin.prototype.onAddQualityLevel = function onAddQualityLevel() {
		...
		var autoPlacement =
			arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'bottom';
		...
		if (sortAscending) {
			levelItems.sort(function (current, next) {
				if (
					(typeof current === 'undefined' ? 'undefined' : _typeof(current)) !== 'object' ||
		...
		if (this._qualityButton) {
			this._qualityButton.createItems = function () {
				...
				return autoPlacement === 'top'
					? [autoMenuItem].concat(levelItems)
					: [].concat(levelItems, [autoMenuItem]);
			};
